...
// ---------------- PAGOS ----------------
export const getPagos = async (usuario) => {
  const path = urlAmbientes();
  const idAlumno = usuario._id;
  ...
  try {
    const response = await fetch(`${path}/api/pagos/${idAlumno}`, requestOptions);
    if (response.status == 401) {
      return false
    }
    const result = await response.json()
...
export const pagoUpdateFetch = async (id, pago) => {
  const path = urlAmbientes();
  const idUsuario = id;
  let raw = pago; 
  console.log(raw)
  ...
  try {
    const response = await fetch(`${path}/api/pagos/${idUsuario}`, requestOptions);
